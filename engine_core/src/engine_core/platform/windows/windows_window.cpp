...
void WindowsWindow::init(const WindowProps& props)
{
    ...
    ENGINE_CORE_INFO("Creating window: {} ({}x{})", props.title, props.width,
        props.height);
...
unsigned int WindowsWindow::get_width() const { return m_props.width; }

unsigned int WindowsWindow::get_height() const { return m_props.height; }
...
void WindowsWindow::set_event_callback(
...
bool WindowsWindow::is_vsync() const { return m_vsync; }
...
void WindowsWindow::swap_buffers() { glfwSwapBuffers(m_window); }

// --- Static GLFW callbacks ---

void WindowsWindow::key_callback(GLFWwindow* window, int key, int /*scancode*/,
    int action, int /*mods*/)
{
...
void WindowsWindow::mouse_button_callback(GLFWwindow* window, int button,
    int action, int /*mods*/)
{
    auto* self = static_cast<WindowsWindow*>(glfwGetWindowUserPointer(window));
    ENGINE_CORE_ASSERT(self,
        "GLFW user pointer is null in mouse_button_callback");
    if (!self || !self->m_event_callback)
        return;
...
void WindowsWindow::cursor_position_callback(GLFWwindow* window, double xpos,
    double ypos)
{
    auto* self = static_cast<WindowsWindow*>(glfwGetWindowUserPointer(window));
    ENGINE_CORE_ASSERT(self,
        "GLFW user pointer is null in cursor_position_callback");
    if (!self || !self->m_event_callback)
        return;
...
void WindowsWindow::scroll_callback(GLFWwindow* window, double xoffset,
    double yoffset)
{
    auto* self = static_cast<WindowsWindow*>(glfwGetWindowUserPointer(window));
...
void WindowsWindow::window_size_callback(GLFWwindow* window, int width,
    int height)
{
    auto* self = static_cast<WindowsWindow*>(glfwGetWindowUserPointer(window));
...
void WindowsWindow::window_close_callback(GLFWwindow* window)
{
    auto* self = static_cast<WindowsWindow*>(glfwGetWindowUserPointer(window));
    ENGINE_CORE_ASSERT(self,
        "GLFW user pointer is null in window_close_callback");
    if (!self || !self->m_event_callback)
        return;
